import { clampChroma, hsl, hsv, hwb, inGamut, oklch, parse, rgb } from "culori";
...
export const toHslvwb = (rgb: RGBColor) => {
	...
	if (Number.isNaN(hue) || hue === undefined) {
		hue = 0;
	}
	if (Number.isNaN(hsvHue) || hsvHue === undefined) {
		hsvHue = 0;
	}
	if (Number.isNaN(oklchHue) || oklchHue === undefined) {
		oklchHue = 0;
	}
