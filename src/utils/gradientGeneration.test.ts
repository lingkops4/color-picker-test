...
describe("OKLCH Gradient Generation", () => {
	describe("Basic Gradient Generation", () => {
		it("should generate smooth gradients when gamutGaps is false", () => {
			const gradient = generateOklchGradient(
				50,
				0.2,
				180,
				"hue",
		...
		it("should vary the correct component in each gradient type", () => {
			const lightnessGradient = generateOklchGradient(
				50,
				0.2,
				180,
				"lightness",
			...
			const chromaGradient = generateOklchGradient(
				50,
				0.2,
				180,
				"chroma",
			...
			const hueGradient = generateOklchGradient(
				50,
				0.2,
				180,
				"hue",
			...
			expect(lightnessGradient).toMatch(
				/oklch\(0\.000.*oklch\(0\.[2-8].*oklch\(1\.000/,
			);

			// Chroma should vary second parameter
			expect(chromaGradient).toMatch(
				/oklch\(0\.500 0\.000.*oklch\(0\.500 0\.0[0-9]/,
			);

			// Hue should vary third parameter
			expect(hueGradient).toMatch(
				/oklch\(0\.500 0\.200 0\).*oklch\(0\.500 0\.200 [0-9]/,
			);
		});
	});
	...
	describe("Gamut Checking", () => {
		it("should respect sRGB-only gamut when showP3 is false", () => {
			const sRGBGradient = generateOklchGradient(
				66.2,
				0.278,
				348,
				"hue",
			...
			const colorStops = (sRGBGradient.match(/oklch\([^)]+\)/g) || []).filter(
				(stop) => !stop.includes("oklch(0.2 0 0)"),
			);
		...
		it("should include P3 colors when showP3 is true", () => {
			const p3Gradient = generateOklchGradient(
				66.2,
				0.278,
				348,
				"hue",
			...
			const p3Colors = (p3Gradient.match(/oklch\([^)]+\)/g) || []).filter(
				(stop) => !stop.includes("oklch(0.2 0 0)"),
			);
			const sRGBColors = (sRGBGradient.match(/oklch\([^)]+\)/g) || []).filter(
				(stop) => !stop.includes("oklch(0.2 0 0)"),
			);
	...
	describe("Step Size and Precision", () => {
		it("should capture important hue values with sufficient step count", () => {
			...
			const gradient72 = generateOklchGradient(
				66.2,
				0.278,
				348,
				"hue",
			...
			const gradient36 = generateOklchGradient(
				66.2,
				0.278,
				348,
				"hue",
		...
		it("should generate more color stops with higher step counts", () => {
			const coarse = generateOklchGradient(
				50,
				0.2,
				180,
				"hue",
	...
	describe("OKLAB Interpolation for Lightness", () => {
		it("should use OKLAB interpolation for lightness gradients in smooth mode", () => {
			const lightness = generateOklchGradient(
				66.2,
				0.278,
				348,
				"lightness",
