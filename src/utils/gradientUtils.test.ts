	...
	describe("background", () => {
		describe("hsl model", () => {
			...
			it("luminosity gradient steps from 0% to 100% (black to white)", () => {
				...
				expect(result).toContain(
					"hsl(var(--picker-hue) calc(var(--picker-saturation) * 1%) 0%)",
				);
				expect(result).toContain(
					"hsl(var(--picker-hue) calc(var(--picker-saturation) * 1%) 100%)",
				);
		...
		describe("rgb model", () => {
			...
			it("green gradient varies from 0 to 255", () => {
				...
				expect(result).toMatch(
					/^linear-gradient\(to right, rgb\(var\(--picker-red\) 0 /,
				);
				expect(result).toMatch(
			...
			it("blue gradient varies from 0 to 255", () => {
				...
				expect(result).toMatch(
					/^linear-gradient\(to right, rgb\(var\(--picker-red\) var\(--picker-green\) 0\)/,
				);
				expect(result).toMatch(
