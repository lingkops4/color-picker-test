 ...
 *
 */
import { inGamut, oklab, oklch, rgb } from "culori";
	...
	if (!gamutGaps) {
		...
		if (component === "lightness") {
			...
			for (let i = 0; i <= steps; i++) {
				...
				// Ensure hue is valid
				const validHue = Number.isNaN(h) || h === undefined ? 0 : h;
		...
		for (let i = 0; i < validColors.length; i++) {
			...
			if (i < validColors.length - 1) {
				...
				if (gap > stepSize * 1.5) {
					...
					gradientStops.push(
						`oklch(0.2 0 0) ${(nextPosition - 0.1).toFixed(1)}%`,
					);
				}
			} else {
